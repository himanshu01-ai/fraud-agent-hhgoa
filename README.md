...
The organisers' task specification is in [docs/HHGOA_DATASET_README.md](docs/HHGOA_DATASET_README.md), committed
verbatim. The Fraud Policy it defines (rules R1–R10, plus sections 2 approval routing, 3a case vs report, and
6 stopping) and the five documented fraud patterns are extracted from it into [rag/](rag/) — those extracts are
what the policy engine cites by rule number in every answer file.

## Team Rules

- **Never open, download, or reference the original public IEEE-CIS / Kaggle Fraud Detection dataset.** All IDs, timestamps, and amounts in this project's dataset have been deliberately disguised versus the public source specifically to prevent answer lookup. Touching the public files, even out of curiosity, risks disqualification per the challenge rules.
...
**Dataset.** The HHGOA_IEEE dataset is provided by the organisers and is **not bundled in this repo** — it is far
too large to commit and we are not free to redistribute it. Obtain it from the organisers and keep it outside the
repo. The scripts expect `transactions*.csv`, `identity.csv`, `closed_cases_history.csv` and `case_pack.csv` all in
one folder, passed as `--data-dir`.
